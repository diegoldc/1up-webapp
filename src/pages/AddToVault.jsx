...
function AddToVault() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const myVaultData = await axios.get(`${import.meta.env.VITE_LOCAL_URL}/myGames/?gameId=${gameId}`)
      console.log("my games" ,myVaultData)
  ...
  return (
    ...
    <Form onSubmit={handleSubmit}>
      ...
      <Form.Select
        ...
        onChange={() => setNewVaultObj({...newVaultObj,platform:`${event.target.value}`})}
        required
        >
        <option value="">Select Platform</option>
        {possiblePlatforms.map((eachPlatform, index) => {
          return <option key={index} value={`${eachPlatform.platform.name}`}>{eachPlatform.platform.name}</option>
        })}
      </Form.Select>
      </Form.Group>
      <Form.Group>
      ...
      <Form.Group>
        ...
        <Form.Control
          ...
          onChange={() => setNewVaultObj({...newVaultObj,hoursPlayed:event.target.value})}
          />

      </Form.Group>
      <button className="button3D" type="submit">Add to Vault</button>
    </Form>
    </>
  )
}
